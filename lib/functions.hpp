...
template <class T> T aperiodic_two_pts_corr_fun(T Z,T M,double TH,double t)
{return Z*Z*exp(-M*TH)*sinh(M*(TH-t))/M;}
//
template <class T> T two_pts_corr_fun(T Z,T M,double TH,double t,const int par)
{
  if(par==1) return  periodic_two_pts_corr_fun(Z,M,TH,t);
  else       return aperiodic_two_pts_corr_fun(Z,M,TH,t);
}
...
//! return the ratio of a corr with insertion and an original corr
template< class T> T aperiodic_two_pts_corr_with_ins_ratio_fun(T M,T A,T SL,double TH,double t)
{return A-SL*(t-TH)/tanh(M*(t-TH));}
template <class T> T two_pts_corr_with_ins_ratio_fun(T M,T A,T SL,double TH,double t,const int par)
{
  if(par==1) return  periodic_two_pts_corr_with_ins_ratio_fun(M,A,SL,TH,t);
  else       return aperiodic_two_pts_corr_with_ins_ratio_fun(M,A,SL,TH,t);
}
