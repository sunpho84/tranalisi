...
template <class TV,class TS=typename TV::base_type> void two_pts_with_ins_ratio_fit(TS &Z2,TS &M,TS &DZ2,TS &SL,const TV &corr,const TV &corr_ins,size_t TH,size_t tmin,size_t tmax,string path="",string path_ins="",int par=1)
{
  //perform a preliminary fit
  ...
  DZ2=constant_fit(eff_slope_offset,tmin,tmax,"/tmp/test_slope_offset.xmg");
  
  if(SL.err()!=0.0 and DZ2.ave()!=0.0)
    {
      //! fit for real
      size_t iel=0;
      ...
      for(iel=0;iel<corr[0].size();iel++)
	{
      ...
      if(path_ins!="") write_fit_plot(path_ins,tmin,tmax,[M,DZ2,SL,TH,par](double x)->TS{return two_pts_corr_with_ins_ratio_fun(M,DZ2,SL,TH,x,par);},TV(corr_ins/corr));
    }
