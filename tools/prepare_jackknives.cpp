...
int main(int narg,char **arg)
{
  ...
  for(auto &it : files)
    {
      auto start=take_time();
      djvec_t data=read_conf_set_t(it.first,file_range,ntot_cols,cols,T,VERBOSE);
      ...
      for(auto &filter : filters)
	{
	  ...
	  vec_filter(data,base_nel*offset,{hw,T*ncols},{each*T*ncols,1}).bin_write(combine(it.second.c_str(),filter.first.c_str()));
