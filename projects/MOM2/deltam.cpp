...
void perens_t::compute_deltam_from_corr()
{
  ...
  for(size_t im=0;im<nm;im++)
	...
	switch(pars::use_QED)
	  ...
	  case 2:
	    {
	      const djvec_t P5P5_LO=get_contraction(im,LO,im,LO,"P5P5",RE,UNK,rfw,rdiff);
	      const djvec_t V0P5_LO=get_contraction(im,LO,im,LO,"V0P5",IM,UNK,rfw,rdiff);
	      const djvec_t V0P5_QED=2.0*get_contraction(im,LO,im,QED,"V0P5",IM,UNK,rfw,rdiff);
	      const djvec_t P5P5_QED=2.0*get_contraction(im,LO,im,QED,"P5P5",RE,UNK,rfw,rdiff);
	      const djvec_t a=djvec_t(symmetric_derivative(V0P5_QED)/P5P5_LO-symmetric_derivative(V0P5_LO)/sqr(P5P5_LO)*P5P5_QED).symmetrized().subset(0,T/2-1);
	      const djack_t dM_pcac=constant_fit(a,tmin,tmax,dir_path+"/plots/var_m_pcac_m"+to_string(im)+"_rfw"+to_string(rfw)+"_P5P5.xmg");
