...
int main()
{
  for(const auto& thT : thTag)
    th.push_back(strtod(thT.c_str(),nullptr));
  
  set_njacks(50);
  
  index_t id3pts({{"m",2},{"th",nth},{"consLoc",2},{"contr",nContrs}});
  ...
  if(file_exists(dataPath))
	...
	{
	  auto get3ptGetter=[ith](const string& consLoc,
				  const int& nmu)
	  {
	    return
	      [data=read_conf_set_t("out/%04d/mes_contr_PHOTON_EMISSION_TH_"+thTag[ith]+"_M0_0.0184_M1_0.237"+consLoc,file_range,2,{0,1},T,VERBOSE),
