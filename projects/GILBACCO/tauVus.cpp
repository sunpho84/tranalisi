...
struct ChannAnalysis
{
  ...
  auto analyze(const djvec_t& fullCorr,
	       ...
	       const double& z) const
  {
    ...
    const auto fitPars=
      multiFit<0>(fullCorr,T,baseOut,tMinStates,tMaxStates,tMinState0,tMaxState0,"state0Fit.xmg",tMaxState1,tMaxState1,"state1Fit.xmg");
      ...
      for(size_t iT=0;iT<nT;iT++)
	{
      ...
      // const double tNormInFm=0.5;
      // const size_t tNorm=round(tNormInFm*aInv.ave()/0.197);
      cout<<"tNorm: "<<tSpecDensNorm<<endl;
      
      /// Ansatz of the spectral density
      const auto specAns=
	[norm=cNorm[tSpecDensNorm].ave(),
	 &enlargedRes](const Real& E)
	{
...
#undef READ
  ...
  readPars(VKVKchann,0,2);
  readPars(AKAKchann,1,2);
  
  VKVKchann.analyze(cVKVK_ls,covVKVK_ls,ensName,T,aInv,z);
  AKAKchann.analyze(cAKAK_ls,covAKAK_ls,ensName,T,aInv,zVlist.at(ensName[0]));
}
