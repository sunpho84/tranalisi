...
dboot_t cont_chir_linear_fit(const dbvec_t &a,const dbvec_t &z,const vector<cont_chir_fit_data_t> &ext_data,const dboot_t &ml_phys,const string &path,const string &yaxis_label,double apow,double zpow,size_t an_flag,bool with_without_FSE,bool cov_flag,const vector<string> &beta_list)
{
  ...
  dboot_t phys_res=cont_chir_linear_ansatz(pars.C,pars.KPi,ml_phys,dboot_t(ext_data[0].aMD/pars.fit_a[0]),a_cont,pars.adep,inf_vol,pars.L3dep,an_flag,with_without_FSE);
  ...
  for(int univ_full_sub=0;univ_full_sub<2;univ_full_sub++)
    plot_chir_fit(combine(path.c_str(),(univ_full_sub==0)?"_univ":"_full"),ext_data,pars,
		  ...
		  {return cont_chir_linear_ansatz<double,double,double>
		      (pars.C.ave(),pars.KPi.ave(),x,dboot_t(ext_data[0].aMD/pars.fit_a[0])[0],pars.fit_a[ib].ave(),pars.adep.ave(),inf_vol,pars.L3dep.ave(),an_flag,with_without_FSE);},		  
		  bind(cont_chir_linear_ansatz<dboot_t,double,dboot_t,double>,pars.C,pars.KPi,_1,dboot_t(ext_data[0].aMD/pars.fit_a[0]),a_cont,pars.adep,inf_vol,pars.L3dep,an_flag,with_without_FSE),
		  ...
		  {if(univ_full_sub==0)
			...
			return dboot_t((without_with_fse?ext_data[idata].wfse:ext_data[idata].wfse)*pow(pars.fit_z[ib],zpow)/pow(pars.fit_a[ib],apow)-
				       without_with_fse*FSE_dep_ML3(pars.L3dep,dboot_t(ext_data[idata].aMD/pars.fit_a[ib]),pars.fit_a[ib],ext_data[idata].L));
		      }
		  },
