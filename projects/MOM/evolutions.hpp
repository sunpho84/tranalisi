...
#define DEFC(IZ,NF,ORD,VAL)			\
  template<>					\
  constexpr double C<ev::IZ,ev::NF,ORD>()	\
  {return VAL;}
...
#define DEFA_NUM(IZ,VAL)			\
  template<>					\
  constexpr double AdimNum<ev::IZ>()		\
  {return VAL;}
...
#define DEFA_DEN(NF,VAL)			\
  template<>					\
  constexpr double AdimDen<ev::NF>()		\
  {return VAL;}
  ...
  for(size_t i=0;i<2;i++)
    {
      ...
      cmu[i]=1.0;
      if(Ord>=1) cmu[i]+=al*C<Iz,Nf,1>();
      if(Ord>=2) cmu[i]+=al*al*C<Iz,Nf,2>();
      if(Ord>=3) cmu[i]+=al*al*al*C<Iz,Nf,3>();
...
//! parse Nf
template<int Iz>
inline double evolution_to_RIp(ev::Nf_t Nf,int ord,double ainv,double mu_2)
{
  double e=0.0;
  
  switch(Nf)
    {
    case ev::NF0:e=evolution_to_RIp<Iz,ev::NF0>(ord,ainv,mu_2);break;
    case ev::NF2:e=evolution_to_RIp<Iz,ev::NF2>(ord,ainv,mu_2);break;
    case ev::NF4:e=evolution_to_RIp<Iz,ev::NF4>(ord,ainv,mu_2);break;
    default:
...
//! parse Iz
inline double evolution_Zbil_to_RIp(iZbil_t iZ,ev::Nf_t Nf,int ord,double ainv,double mu_2)
{
  double e=0.0;
  ...
  switch(iZ)
    ...
    case iZbil_t::iZS:
      //falltrhough
    case iZbil_t::iZP:
      e=evolution_to_RIp<ev::S>(Nf,ord,ainv,mu_2);
      break;
    case iZbil_t::iZT:
      e=evolution_to_RIp<ev::T>(Nf,ord,ainv,mu_2);
      break;
    case iZbil_t::iZV:
      //falltrhough
    case iZbil_t::iZA:
...
//! parse Zq
inline double evolution_Zq_to_RIp(ev::Nf_t Nf,int ord,double ainv,double mu_2)
{
  return evolution_to_RIp<ev::Q>(Nf,ord,ainv,mu_2);
}
