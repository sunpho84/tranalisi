...
djvec_t read(const char *SM_SI,const char *SM_SO,const int iproj,const int iWick)
{
  string path=combine("jacks/bar_alt_contr_%sPPP%s_proj_%d_Wick_%d",SM_SI,SM_SO,iproj,iWick);
...
djvec_t read_phi()
{
  ...
  for(int i=1;i<=3;i++)
    phi_ss+=read_djvec(combine("jacks/mes_contr_SPS_SPS_V%dV%d",i,i),T);
...
int main()
{
  ...
  const djvec_t dir_55=read(SM_SI,SM_SO,0,0);
  const djvec_t exc_55=read(SM_SI,SM_SO,0,1);
  const djvec_t dir_ii=read(SM_SI,SM_SO,1,0);
  const djvec_t exc_ii=read(SM_SI,SM_SO,1,1);
  const djvec_t dir_ij=read(SM_SI,SM_SO,2,0);
  const djvec_t exc_ij=read(SM_SI,SM_SO,2,1);
