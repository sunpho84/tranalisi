...
djvec_t load(const string& bfName,
  ...
  for(size_t iConf=0;iConf<nConfs;iConf++)
    {
      ...
      const filesystem::path bfPath="data/"+confs[iConf]+"/"+bfName+".bin";
      if(filesystem::exists(bfPath))
	{
	  binFile.open(bfPath.c_str(),"r");
	  binFile.bin_read(sum);
	  //cout<<comprPath<<" read"<<endl;
	}
      else
	{
	  ...
	  for(size_t r=0;r<2;r++)
	    for(const auto& [q1,q2] : list)
	      {
		raw_file_t in(combine("/home/francesco/QCD/LAVORI/GM4/bubbles/E112/data/%s/mes_contr_OET_%s%s_R%zu",confs[iConf].c_str(),q1,q2,r),"r");
		line_t line;
		...
		for(size_t icopy=0;icopy<nCopies;icopy++)
		  for(size_t ihit=0;ihit<nHits;ihit++)
			...
			CRASH("%s %d %d     %s %s",confs[iConf].c_str(),ab[0],ab[1],firstProp,secondProp);
		      // printf("%s %s %d %d\n",q1,q2,ab[0],ab[1]);
	  ...
	  binFile.open(bfPath.c_str(),"w");
	  binFile.bin_write(sum);
	  //cout<<comprPath<<" written"<<endl;
	}
...
int main()
{
  ...
  const djvec_t l234s=load("l234s",{{"l","l2"},{"l2","l3"},{"l3","l4"},{"l4","s"}});
  const djvec_t ls=load("ls",{{"l","s"}});
  l234s.ave_err().write("plots/l234s.xmg");
  ls.ave_err().write("plots/ls.xmg");
